          ...
          <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-1.5">
              <Button
                ...
                title="Save this chat as an itinerary"
              >
                <Bookmark className="h-3.5 w-3.5" /> <span className="hidden sm:inline text-xs">Save</span>
              </Button>
                  ...
                  <div className="flex gap-1 p-0.5 bg-muted/60 rounded-lg">
                    <button
                      ...
                      className={`flex-1 text-[11px] font-medium py-1.5 rounded-md transition-colors ${
                        historyTab === "chats" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"
                      }`}
                    >
                      Chats
                    </button>
                    <button
                      onClick={() => setHistoryTab("itineraries")}
                      className={`flex-1 text-[11px] font-medium py-1.5 rounded-md transition-colors ${
                        historyTab === "itineraries" ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"
                      }`}
                    >
                      Itineraries
                    </button>
                    ...
                    <>
                      {itineraries.length === 0 && (
                        <p className="text-xs text-muted-foreground px-1">
                          No saved itineraries yet. Use Save to keep a chat.
                        </p>
                      )}
                      {itineraries.map((it) => (
                        <div
                          key={it.id}
                          className="group flex items-start gap-1 rounded-lg px-2 py-2 hover:bg-muted/60"
                        >
                          <button
                            onClick={() => openItinerary(it.id)}
                            className="flex-1 text-left min-w-0"
                          >
                            <div className="text-xs font-medium truncate text-foreground">{it.title}</div>
                            <div className="text-[10px] text-muted-foreground/70 mt-0.5">
                              {new Date(it.savedAt).toLocaleDateString()} · {it.messages.length} msgs
                            </div>
                          </button>
                          <button
